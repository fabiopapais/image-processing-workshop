...
void challenge(int height, int width, RGBTRIPLE image[height][width])
{
  ...
  int x_quarter_limit = round(width / 2);
  ...
  {
    // Canto inferior esquerdo (1/4)
    for (int x = 0; x < x_quarter_limit; x++)
    {
      sepia(height, width, image, tmp_image, y, x);
    }

    // Canto inferior direito (2/4)
    for (int x = x_quarter_limit; x < width; x++)
    {
      grayscale(height, width, image, tmp_image, y, x);
    }
  }
