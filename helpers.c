...
void challenge(int height, int width, RGBTRIPLE image[height][width])
{
  ...
  // *************************
  // E AQUI VOCÊ FAZ A MÁGICA!
  // *************************
  
  // Dicas:
  // - Observe que as funções neste arquivo possuem argumentos diferentes dos exemplos anteriores
  // - O código (como pode ser visto) sugere o uso de uma tmp_image, então tome cuidado com isso
  // - Sempre que encontrar um erro, veja ele desde o primeiro até o último, resolvendo-os aos poucos
  // - Boa sorte ;) Vai ser massa!
