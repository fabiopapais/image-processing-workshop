...
void sepia(int height, int width, RGBTRIPLE image[height][width])
{
  ...
  for (int y = 0; y < height; y++)
  {
    ...
    {
      float sepiaRed = 0.1 * image[y][x].rgbtRed;
      float sepiaGreen = 0.8 * image[y][x].rgbtGreen;
      float sepiaBlue = 1.3 * image[y][x].rgbtBlue;
...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
  ...
  // copying pixels from original image to tmp_image
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
  ...
  // Here we play with the sizes and positions
  for (int y = 0; y < 150; y++)
  {
    for (int x = 400; x < width - 250; x++)
    {
      float sumRed = 0;
      float sumGreen = 0;
      float sumBlue = 0;
...
void edge(int height, int width, RGBTRIPLE image[height][width])
{
  ...
  // copying pixels into original image
  /*
    There several ways to do this, a simple approach is:
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
    ...
    But after running the program, we can perceive that a white-colored border appears on the image :(
    It happens because our kernels will always recognize the as borders, because not all the parts of
    the kernel will be recognized. 
